...
export class DailyCompetitionFinalizationService {
  async finalizeDailyCompetition(competitionId: string): Promise<void> {
    try {
      logger.info('Finalizando competição diária (nova dinâmica - sem ranking diário)...', { 
        competitionId 
      ...
      if (!competition.weekly_tournament_id) {
        logger.error('Competição diária não está vinculada a uma competição semanal', { 
          competitionId 
        }, 'DAILY_COMPETITION_FINALIZATION');
        return;
      }
      ...
      logger.info('Rankings da competição semanal atualizados', { 
        weeklyTournamentId: competition.weekly_tournament_id 
      }, 'DAILY_COMPETITION_FINALIZATION');
      ...
      logger.info('Competição diária finalizada com sucesso (pontos já na competição semanal)', { 
        competitionId 
      }, 'DAILY_COMPETITION_FINALIZATION');
    } catch (error) {
      logger.error('Erro ao finalizar competição diária', { 
        competitionId, 
        error 
      }, 'DAILY_COMPETITION_FINALIZATION');
    }
  ...
  async transferScoresToWeeklyCompetition(dailyCompetitionId: string): Promise<void> {
    try {
      logger.info('Com a nova dinâmica, não há transferência de pontos - os pontos já são contabilizados diretamente na competição semanal', { 
        dailyCompetitionId 
      }, 'DAILY_COMPETITION_FINALIZATION');
      ...
      if (dailyError || !dailyCompetition?.weekly_tournament_id) {
        logger.warn('Competição diária não vinculada a competição semanal', { 
          dailyCompetitionId 
        }, 'DAILY_COMPETITION_FINALIZATION');
        return;
      }
      ...
      logger.error('Erro ao verificar vinculação', { 
        dailyCompetitionId, 
        error 
      }, 'DAILY_COMPETITION_FINALIZATION');
    }
