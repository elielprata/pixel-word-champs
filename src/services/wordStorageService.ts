...
export const saveWordsToDatabase = async (
  ...
  // CORREÇÃO: Verificar quais palavras já existem no banco APENAS na mesma categoria
  const { data: existingWords, error: checkError } = await supabase
    .from('level_words')
    .select('word')
    .in('word', normalizedWords)
    .eq('category', categoryName)  // 🔧 ADICIONADO: filtrar por categoria
    .eq('is_active', true);
  ...
  // Criar um Set das palavras que já existem NA MESMA CATEGORIA
  const existingWordsSet = new Set(
    existingWords?.map(item => item.word) || []
  );

  // Filtrar apenas palavras que realmente não existem na categoria atual
  ...
  for (const wordData of wordsToInsert) {
    try {
      ...
      if (error) {
        ...
        if (error.code === '23505') {
          console.log(`⚠️ Palavra já existe (ignorando): ${wordData.word} na categoria ${categoryName}`);
        } else {
        ...
        successCount++;
        console.log(`✅ Palavra inserida: ${wordData.word} na categoria ${categoryName}`);
      }
    } catch (err) {
