  ...
  return (
    <div className="p-2 space-y-2">
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-1.5">
          <div className="relative">
            ...
            <Sparkles className="w-1.5 h-1.5 text-yellow-500 absolute -top-0.5 -right-0.5 animate-pulse" />
          </div>
          <span className="text-xs font-bold bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent">
            Palavras
          </span>
        </div>
        <div className="flex items-center gap-1 bg-gradient-to-r from-purple-100 to-blue-100 px-2 py-0.5 rounded-full border border-purple-200">
          <span className="text-xs font-bold text-purple-700">{foundWords.length}</span>
          <span className="text-xs text-purple-500 mx-0.5">/</span>
          <span className="text-xs text-purple-600">{TOTAL_WORDS}</span>
        </div>
      </div>
      
      {/* Grid de palavras ultra compacto */}
        ...
        {levelWords.map((word, index) => {
          ...
          return (
            <div 
              key={index}
              className={`
                relative flex items-center justify-between px-2 py-1.5 rounded-lg transition-all duration-300 transform hover:scale-[1.01] border
                ${isFound 
                  ? `bg-gradient-to-r ${getWordColor(foundWordIndex)} text-white shadow-md border-white/30 animate-bounce-in` 
                  : isHidden
            ...
            >
              ...
              <div className="flex items-center gap-1.5 flex-1 min-w-0">
                ...
                {isHidden && !isFound && (
                  <Lock className="w-3.5 h-3.5 text-white/90 flex-shrink-0" />
                )}
              ...
              {/* Pontos ou indicador compacto */}
              <div className="flex items-center gap-1 flex-shrink-0">
                {isFound && foundWord && (
                  <span className="text-xs font-bold text-white bg-black/20 px-1.5 py-0.5 rounded-full border border-white/20">
                    +{foundWord.points}
                  </span>
                )}
                {isHidden && !isFound && (
      ...
      {foundWords.length === TOTAL_WORDS && (
        <div className="flex items-center justify-center gap-1.5 px-3 py-2 bg-gradient-to-r from-emerald-500 via-green-500 to-emerald-600 text-white rounded-lg shadow-md border border-green-300 animate-bounce-in">
          <Star className="w-3.5 h-3.5 animate-spin" />
          <span className="text-xs font-bold">Nível Completo!</span>
          <Star className="w-3.5 h-3.5 animate-spin" style={{ animationDelay: '0.5s' }} />
