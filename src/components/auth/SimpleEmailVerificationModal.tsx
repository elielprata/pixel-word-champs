...
interface SimpleEmailVerificationModalProps {
  isOpen: boolean;
  onClose: () => void;
  userEmail: string;
}

export const SimpleEmailVerificationModal = ({ 
  isOpen, 
  onClose, 
  userEmail 
}: SimpleEmailVerificationModalProps) => {
  const [isResending, setIsResending] = React.useState(false);
  const { toast } = useToast();

  console.log('🔍 [DEBUG] SimpleEmailVerificationModal renderizado:', { isOpen, userEmail });
  ...
  if (!isOpen) {
    console.log('🔍 [DEBUG] Modal não está aberto, não renderizando');
    return null;
  }

  console.log('🔍 [DEBUG] Modal ESTÁ ABERTO, renderizando com Portal');
    ...
    >
      <div 
        style={{
          ...
          border: '3px solid #10b981' // DEBUG: Border verde para visualizar
        }}
