...
const CompetitionCard = ({ competition, onJoin }: CompetitionCardProps) => {
  // Usar status diretamente do backend (já corrigido pelo cron job)
  const backendStatus = competition.status;
  
  // Calcular tempo restante baseado na data de fim
  ...
  const timeDisplay = useMemo(() => {
    if (backendStatus === 'completed') return 'Finalizada';
    if (backendStatus === 'scheduled') return 'Em breve';
    ...
    return `${minutes}m`;
  }, [backendStatus, timeRemaining]);

  // Só mostrar se for ativa ou agendada (confiar no backend)
  ...
  return (
    <Card className={`relative border-0 bg-gradient-to-br ${bgGradient} backdrop-blur-sm shadow-lg hover:shadow-2xl transition-all duration-500 hover:scale-[1.03] animate-fade-in group overflow-hidden`}>
      ...
      <CardContent className="p-4 relative">
            ...
            <div>
              ...
              <div className="mt-1">
                <CompetitionStatusBadge 
                  ...
                  isRealTime={backendStatus === 'active'} 
        ...
        {/* Botão de ação baseado no status do backend */}
