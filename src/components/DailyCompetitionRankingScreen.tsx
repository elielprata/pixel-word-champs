...
interface DailyCompetitionRankingScreenProps {
  competitionId: number;
  onBack: () => void;
}

const DailyCompetitionRankingScreen = ({ competitionId, onBack }: DailyCompetitionRankingScreenProps) => {
  const competitionData = {
    1: { title: "Competição Matinal", description: "Palavras relacionadas ao café da manhã" },
  ...
  }[competitionId] || { title: "Competição Diária", description: "Ranking da competição" };
  ...
  return (
    <div className="p-4 pb-20 bg-gradient-to-b from-purple-50 to-blue-50 min-h-screen">
      <div className="flex items-center mb-6">
        ...
        <div className="ml-3">
          <h1 className="text-xl font-bold text-purple-800">{competitionData.title}</h1>
          <p className="text-sm text-gray-600">{competitionData.description}</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Ranking da Competição</CardTitle>
        </CardHeader>
        <CardContent>
...
export default DailyCompetitionRankingScreen;
