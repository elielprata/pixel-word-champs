...
-- 1.1 Analisar triggers que podem estar causando overhead
-- Remover triggers de logging desnecessários ou redundantes
DROP TRIGGER IF EXISTS log_session_creation_trigger ON public.game_sessions;

-- Manter apenas triggers essenciais para funcionalidade
-- Verificar se trigger de atualização de ranking é eficiente
DROP TRIGGER IF EXISTS trigger_weekly_ranking_update ON public.profiles;

-- Recriar trigger otimizado para ranking (apenas quando necessário)
...
DECLARE
  ...
  cache_key text;
BEGIN
  -- Criar chave de cache baseada no user_id e timestamp
  cache_key := 'admin_' || COALESCE(auth.uid()::text, 'null');
  
  -- Verificar cache (simular com variável de sessão)
  BEGIN
    admin_status := current_setting('app.admin_cache_' || cache_key, true)::boolean;
    IF admin_status IS NOT NULL THEN
      RETURN admin_status;
    END IF;
  EXCEPTION WHEN OTHERS THEN
    -- Cache miss, continuar para consulta
  END;
  ...
  -- Definir cache por 5 minutos (300 segundos)
  BEGIN
    PERFORM set_config('app.admin_cache_' || cache_key, admin_status::text, false);
  EXCEPTION WHEN OTHERS THEN
    -- Falha no cache não deve impedir a função
  END;
...
-- 3.1 Índices compostos para consultas mais comuns
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_score_active 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_invites_active_usage 
ON public.invites(invited_by, used_by, created_at) 
WHERE is_active = true AND used_by IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_word_history_recent 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_roles_admin_lookup 
