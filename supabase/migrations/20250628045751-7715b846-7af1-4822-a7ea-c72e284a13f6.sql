...
-- Corrigir referência ambígua da coluna total_participants na função get_monthly_invite_stats
...
DECLARE
  ...
  participants_count integer := 0;  -- Renomeado para evitar conflito
  rankings_data jsonb;
...
BEGIN
  ...
  INTO rankings_data, participants_count, calculated_prize_pool
  FROM final_rankings;
  ...
  -- Atualizar total_prize_pool na competição (usando variável renomeada)
  ...
  IF COALESCE(participants_count, 0) = 0 THEN
