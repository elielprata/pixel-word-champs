...
-- Atualizar todas as competições existentes para usar a nova terminologia
UPDATE custom_competitions 
SET competition_type = 'daily' 
WHERE competition_type = 'challenge';

UPDATE custom_competitions 
SET competition_type = 'weekly' 
WHERE competition_type = 'tournament';

-- Atualizar as funções do banco que referenciam os tipos antigos
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR 
    ...
    -- Para competições diárias: forçar início às 03:00:00 UTC (00:00 Brasília) 
    -- e fim às 02:59:59 UTC do dia seguinte (23:59:59 Brasília)
    IF NEW.competition_type = 'daily' THEN
    ...
    -- Para competições semanais: forçar início às 03:00:00 UTC (00:00 Brasília) 
    -- e fim às 02:59:59 UTC (23:59:59 Brasília)
    IF NEW.competition_type = 'weekly' THEN
      NEW.start_date := DATE_TRUNC('day', NEW.start_date) + INTERVAL '3 hours';
...
-- Atualizar função de status das competições diárias
...
-- Atualizar função de status das competições semanais
