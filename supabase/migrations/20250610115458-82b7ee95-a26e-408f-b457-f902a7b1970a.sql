...
-- Primeiro, vamos verificar se a função update_weekly_ranking existe e tem algum problema
-- Vamos recriar a função com tratamento de erros melhorado
...
BEGIN
  ...
  -- Verificar se o enum payment_status existe, se não existir, criar
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'payment_status') THEN
      CREATE TYPE payment_status AS ENUM ('pending', 'paid', 'failed', 'not_eligible');
      RAISE NOTICE 'Enum payment_status criado';
    END IF;
  END $$;
